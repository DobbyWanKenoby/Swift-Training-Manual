/*:
 ...
 Используется для одновременного (синхронного или асинхронного) выполнения нескольких задач.
 
 ### Основные понятия
 
 * __Thread (поток)__ - последовательность инструкций, которая может быть выполнена независимо от отслального кода программы.
 * __Queue (очередь)__ - множество выполняемых задач, абстракция, созданная Apple в GCD и operation, позволяющая не опускаться на уровень потоков.
 ...
 * __Concurrent (async)__ - Текущйи поток продолжает выполнение паралельно с другим потоком.
 ...
 ### Варианты реализации мультипоточности в Swift
 ...
 * Другие
    ...
    * objc_sync
    * spin lock - deprecated in iOS 10, заменена unfair lock

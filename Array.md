...
Во многих из перечисленных методов используется сокращенный синтаксис для доступа к элементам вид `$N`, где `N` - целое число (например, `$0`). С помощью значений такого вида внутри замыканий осуществляется доступ к входным параметрам: `$0` - к первому, `$1` - ко второму и т.д.
...
Существует нсеколкьо способов создания массивов: с помощью полного или облегченного синтаксиса, а также используя различные инициализаторы.
...
--- | --- | --- | ---
`subscript` | Возвращает элемент по его индексу | O(1) | `sourceArray[2]` |
`count` | Возвращает количество элементов в массиве. | O(1) | `sourceArray.count` |
`first` | Возвращает первый элемент (опционал) |  | `sourceArray.first` |
`first(where:)` | Возвращает первый элемент, соответствующий условию (опционал) |  | `sourceArray.first(where: { $0 > 3 })` |
`last` | Возвращает последний элемент (опционал) |  | `sourceArray.last` |
`last(where:)` | Возвращает последний элемент, соответствующий условию (опционал) |  | `sourceArray.дфые(where: { $0 > 3 })` |
`max()` | Возвращает максимальный элемент (опционал)|  | `sourceArray.max` |
`max(by:)` | Возвращает максимальный элемент с предварительной сортировкой (опционал)|  | `let man = structArray.max(by: { $0.age < $1.age })` |
`min()` | Возвращает минимальный элемент (опционал)|  | `sourceArray.max` |
`min(by:)` | Возвращает минимальный элемент с предварительной сортировкой (опционал)|  | `let man = structArray.min(by: { $0.age < $1.age })` |
